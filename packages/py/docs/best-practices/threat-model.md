...
- **JWT signing key** (`AuthConfig.secret_key`) -- the single secret protecting all tokens.
...
- **Secure cookie defaults.** `AuthConfig` sets all cookie flags defensively out of the box:
...
- **CSRF middleware available.** `AuthConfig.csrf_enabled` can be set to `True` to enable double-submit cookie CSRF protection via `csrf_cookie_name` and `csrf_header_name`.
...
- **`AuthConfig.secret_key`** -- the HMAC signing key (or PEM key for RSA/EC).
...
- **Default key raises `ValueError`.** If `secret_key` is still `"CHANGE-ME-IN-PRODUCTION"` and `allow_insecure_key` is `False` (the default), `AuthConfig` raises immediately at startup:
...
- **Environment-based configuration.** `AuthConfig` extends `pydantic-settings.BaseSettings` with `env_prefix="AUTH_"`, encouraging `AUTH_SECRET_KEY` as an environment variable rather than a hardcoded value.
