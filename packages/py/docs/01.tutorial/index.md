...
|------|-------------------|
| [First Steps](first-steps.md) | Install, create a user model, subclass `Auth` with user-data methods, wire up `FastAuth`, test with curl |
...
| [Auth Configuration](pipeline.md) | Auth methods (`JWT`, `Session`, etc.), login methods (`Password`, `OTP`, etc.), `auto_router()` |
| [OAuth2 & Social Login](oauth2-social-login.md) | Google, GitHub, and other providers via `OAuth` |
