...
## Pipeline-Based Setup

The recommended way to configure OAuth is through the `Pipeline`:

```python
from urauth.pipeline import Pipeline, OAuthLogin, Google
...
When you call `auth.auto_router()`, it reads the pipeline and generates OAuth endpoints for each configured provider:
...
For a pipeline with Google OAuth, this creates:
...
The callback path is configurable via `OAuthLogin(callback_path="/auth/oauth/{provider}/callback")`.
...
All providers are imported from `urauth.pipeline`:

```python
from urauth.pipeline import Google, GitHub, Microsoft, Apple, Discord, GitLab
```
...
```python
from urauth.pipeline import OAuthProvider
...
pipeline = Pipeline(
    oauth=OAuthLogin(providers=[custom]),
)
```
...
- Configure providers in `Pipeline(oauth=OAuthLogin(providers=[...]))`.
- Override `get_or_create_oauth_user(info)` to resolve OAuth identities to local users.
