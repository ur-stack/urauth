...
"""SQLModel user-data mixin and auth factory.
...
Factory shorthand (equivalent)::
    ...
    from urauth.contrib.sqlmodel import create_sqlmodel_auth

    core = create_sqlmodel_auth(
        session_factory=async_session_factory,
        user_model=User,
        secret_key="...",
        method=JWT(ttl=900, store=MemoryTokenStore()),
...
class SQLModelUserStore(UserDataMixin):
    """User-data mixin backed by SQLModel async sessions.
...
def create_sqlmodel_auth(
    ...
    **kwargs: Any,
    ...
    """Create an Auth instance wired to SQLModel async queries.

    Equivalent to ``class _Auth(Auth, SQLModelUserStore): pass`` followed
    by instantiation with all arguments forwarded.

    Args:
        session_factory: An ``async_sessionmaker`` for creating database sessions.
        user_model: Your SQLModel table class.
    ...
    """
    class _SQLModelAuth(Auth, SQLModelUserStore):
        pass

    return _SQLModelAuth(
