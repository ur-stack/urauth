...
class TestNormalRefreshRotation:
    ...
    async def test_old_refresh_token_revoked_after_rotation(self, client: AsyncClient) -> None:
        ...
        # First rotation succeeds (cookie is updated automatically)
        await client.post("/auth/refresh")

        # Second use of old refresh token via body → reuse detected
...
class TestRefreshTokenReplayAttack:
    async def test_replay_revokes_entire_family(self, client: AsyncClient) -> None:
        ...
        # Step 1: Login → pair 1; capture the refresh token value before rotation
        result = await login(client, "admin@test.com", "admin-pass")
        stolen_refresh = result["response"].cookies.get("refresh_token")

        # Step 2: Legitimate rotation via cookie (client jar is updated)
        resp = await client.post("/auth/refresh")
        assert resp.status_code == 200
        pair2 = resp.json()

        # Step 3: Attacker replays stolen refresh token via body (bypassing cookie)
    ...
    async def test_replay_does_not_affect_other_family(self, client: AsyncClient) -> None:
        ...
        # Login session A; save the refresh token before it gets overwritten
        result_a = await login(client, "admin@test.com", "admin-pass")
        stolen_refresh_a = result_a["response"].cookies.get("refresh_token")

        # Login session B (cookie jar now holds session B's refresh token)
        result_b = await login(client, "admin@test.com", "admin-pass")
        token_b = result_b["body"]["access_token"]

        # Rotate session A via body, then replay via body
